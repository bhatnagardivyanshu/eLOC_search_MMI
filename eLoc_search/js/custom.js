			...
			$.ajax({
			        ...
			        success: function(result) {
			        	if(!$.isEmptyObject(result)){
			        		...
			        		/*Center to left*/
			        		$("#search_box").fadeOut(1).removeClass("col-xs-offset-4 col-xs-4");
			        		$("#search_box").addClass("col-xs-2").fadeIn(400);


			        		// $("#search_box").css('padding', '2em');
			        		$("#main_row").removeClass("box_ctr");
			        		$("#result_col").fadeIn(350);
...
function make_changes(){
	...
	$("#submit_form").click(function(){
		...
		//if 2 files are uploaded
		if($("#ins_file")[0].files.length && $("#upd_file")[0].files.length){
	 		alert("Please select one option at a time!");
	 	}
		
	 	else if($("#ins_file")[0].files.length){
...
function make_changes_cancel_btn(){
	$(".modal form").eq(0)[0].reset();
	$(".modal form").eq(1)[0].reset();
}
